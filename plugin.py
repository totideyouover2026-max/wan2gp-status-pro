...
class _TaskOutcomeTelemetry:
    ...
    def capture_outputs(self, task_id, execution_epoch, output_records):
        with self._lock:
            ...
            if record is None:
                return
            by_path = {str(item.get("path") or ""): dict(item) for item in record["output_records"]}
            for item in output_records or []:
                if isinstance(item, dict) and item.get("path"):
                    by_path[str(item["path"])] = dict(item)
            record["output_records"] = list(by_path.values())
            self._revision += 1
    ...
    def finish(self, task_id, execution_epoch, success, aborted=False, error=None, output_records=None):
        with self._lock:
            ...
            self._revision += 1
            record["revision"] = self._revision
        self.capture_outputs(task_id, execution_epoch, output_records)
    ...
    def snapshot(self):
        with self._lock:
            return [{**item, "output_records": [dict(output) for output in item["output_records"]]}
...
def _effective_media_value(value):
    ...
    if isinstance(value, dict):
        # Gradio FileData placeholders contain a truthy ``meta._type`` even when
        # every actual payload carrier is empty.  Only inspect media carriers.
    ...
    # PIL images and Gradio file objects are genuine values; scalar defaults are not.
...
def _generation_output_records(state):
    state = state if isinstance(state, dict) else {}
    gen = state.get("gen") if isinstance(state.get("gen"), dict) else {}
    return _output_records(gen.get("file_list"), gen.get("file_settings_list")) + _output_records(
        gen.get("audio_file_list"), gen.get("audio_file_settings_list"), audio_hint=True
    )
...
class StatusProPlugin(WAN2GPPlugin):
    ...
    def _install_generation_timing_observer(self):
        ...
        def observed(task, *args, **kwargs):
            ...
            output_reader = globals().get("_generation_output_records")
            initial_paths = {record["path"] for record in output_reader(state)} if callable(output_reader) else set()

            def task_outputs():
                if not callable(output_reader):
                    return []
                return [record for record in output_reader(state)
                        if record.get("path") not in initial_paths]
            ...
            try:
                ...
                if task_outcomes is not None:
                    task_outcomes.finish(
                        task_id, execution_epoch, bool(result), aborted=bool(state_gen.get("abort")),
                        error=error, output_records=task_outputs(),
                    )
...
`;
    ...
    function authoritativeOutcomeStatus(outcome, fallback = "completed") {
        if (!outcome || outcome.known !== true) return fallback;
        if (outcome.aborted === true) return "aborted";
        return outcome.success === true ? "completed" : "failed";
    }

    function reconcileTaskOutcomes(namespace, telemetry) {
        const outcomes = Array.isArray(telemetry && telemetry.task_outcomes) ? telemetry.task_outcomes : [];
        let changed = false;
        namespace.runHistory.forEach(run => {
            const outcome = taskOutcomeForRun(run, telemetry);
            if (!outcome || outcome.known !== true) return;
            const status = authoritativeOutcomeStatus(outcome, run.status);
    ...
    function finishRun(namespace, status, completedAt, telemetry, outputEnd) {
        ...
        finishHistoryPhases(namespace.state, outcome, browserStageEnded);
        ...
        run.execution_epoch = optionalNumber(run._stageTimingEpoch);
        normalizeRunMedia(run);
        run.status = outcome;
